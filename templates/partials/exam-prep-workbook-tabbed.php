					...
					<?php if ( 'practice' === $tab_type ) : ?>
						...
						if ( class_exists( 'CTA_Exam_Prep_Workbooks' ) ) {
							foreach ( $cards as $probe_card ) {
								if ( CTA_Exam_Prep_Workbooks::is_chapter_test_quiz( $probe_card['quiz'] ?? null ) ) {
									++$chapter_test_count;
								}
							}
						}
						$has_chapter_tests = $chapter_test_count > 0;
						...
						// Multi chapter-test workbooks: section title is an umbrella; each list item is unique.
						if ( $has_chapter_tests && count( $cards ) > 1 && class_exists( 'CTA_Exam_Prep_Workbooks' ) ) {
							$wb_num_label = CTA_Exam_Prep_Workbooks::workbook_number_from_quiz( $cards[0]['quiz'] ?? null );
							if ( $wb_num_label <= 0 && class_exists( 'CTA_Exam_Prep_Lessons' ) ) {
								$wb_num_label = CTA_Exam_Prep_Lessons::workbook_number_from_module( $module ?? null );
							}
							$wb_bank_label = $wb_num_label > 0
								? sprintf(
									/* translators: %d: workbook number */
									__( 'Workbook %d Chapter Tests', 'cta-lms' ),
									$wb_num_label
								)
								: __( 'Chapter Tests', 'cta-lms' );
						} elseif ( 1 === count( $cards ) && class_exists( 'CTA_Exam_Prep_Workbooks' ) ) {
							$wb_bank_label = CTA_Exam_Prep_Workbooks::get_workbook_practice_bank_button_label(
								$module ?? null,
								$cards[0]['quiz'] ?? null
							);
						}
						$wb_bank_tag = $has_chapter_tests
							? __( 'Chapter Tests', 'cta-lms' )
							...
							<p class="cta-ep-workbook-section__status" data-practice-bank-status="<?php echo esc_attr( $pb_status ); ?>">
								...
								<span class="cta-ep-workbook-section__status-hint">
									...
									echo $has_chapter_tests
										...
										: esc_html__( 'Practice Bank progress (separate from workbook completion)', 'cta-lms' );
									?>
								</span>
							</p>
