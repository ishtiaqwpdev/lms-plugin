...
if ( ! function_exists( 'cta_lms_is_plugin_lifecycle_request' ) ) {
	 ...
	 *
	 * Heavy heal/sync must not run here — Hostinger white-screens update.php.
	 *
	 * @return bool
	 */
	function cta_lms_is_plugin_lifecycle_request() {
		...
		if ( in_array( $action, array( 'upload-plugin', 'upload-theme', 'install-plugin', 'update-plugin', 'activate-plugin', 'activate', 'do-plugin-upgrade', 'upgrade-plugin' ), true ) ) {
			return true;
		}
...
if ( ! function_exists( 'cta_lms_queue_heavy_upgrades_for_version' ) ) {
	...
	function cta_lms_queue_heavy_upgrades_for_version( $installed ) {
		...
		// CE-003 / Alcoholism CE: detach Law & Ethics R9A study toolkits incorrectly linked to CE courses.
		if ( version_compare( $installed, '1.0.314', '<' ) ) {
			cta_lms_queue_deferred_upgrade( 'ce_materials_repair' );
		}
		if ( version_compare( $installed, '1.0.315', '<' ) ) {
			cta_lms_queue_deferred_upgrade( 'ce_materials_repair' );
		}
		if ( version_compare( $installed, '1.0.316', '<' ) ) {
			cta_lms_queue_deferred_upgrade( 'ce_materials_repair' );
		}
...
if ( ! function_exists( 'cta_maybe_upgrade_db' ) ) {
	...
	function cta_maybe_upgrade_db() {
		...
		try {
			...
			// LMFT California Law & Ethics: publish 9 workbook online Practice Banks from approved seeds.
			if ( version_compare( $installed, '1.0.301', '<' ) ) {
				cta_lms_queue_deferred_upgrade( 'lmft_law_ethics_workbook_banks' );
			}

			// LMFT California Law & Ethics: publish Practice A/B + Comprehensive Final online exams.
			// Re-queue through 1.0.307 — prior gates could stamp without a successful write
			// when only deferred admin/cron processing ran (often never on frontend hosts).
			if ( version_compare( $installed, '1.0.310', '<' ) ) {
				cta_lms_queue_deferred_upgrade( 'lmft_law_ethics_practice_exams' );
			}
			if ( class_exists( 'CTA_Lmft_Law_Ethics_Sync' ) && ! CTA_Lmft_Law_Ethics_Sync::practice_exams_are_live() ) {
				cta_lms_queue_deferred_upgrade( 'lmft_law_ethics_practice_exams' );
			}
			if ( class_exists( 'CTA_Lmft_Clinical_Sync' ) && ! CTA_Lmft_Clinical_Sync::workbook_banks_are_live() ) {
				cta_lms_queue_deferred_upgrade( 'lmft_clinical_workbook_banks' );
