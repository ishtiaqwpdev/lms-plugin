	...
	<?php elseif ( 'all_learner_content_published' === $notice ) : ?>
		<div class="notice notice-success is-dismissible">
			<p>
				<?php
				printf(
					/* translators: 1: courses published, 2: practice exams written, 3: remaining queue */
					esc_html__( 'Learner content publish started. Courses set published: %1$d. Law & Ethics Practice Exams written: %2$d. Remaining background tasks: %3$d — stay on admin and refresh every 20–30 seconds until the queue clears.', 'cta-lms' ),
					absint( wp_unslash( $_GET['courses_published'] ?? 0 ) ),
					absint( wp_unslash( $_GET['practice_written'] ?? 0 ) ),
					absint( wp_unslash( $_GET['queued'] ?? 0 ) )
				);
				?>
			</p>
		</div>
