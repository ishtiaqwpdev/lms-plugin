...
<<<<<<< HEAD
	<?php elseif ( 'lmft_le_practice_exams_published' === $notice ) : ?>
		<div class="notice notice-success is-dismissible"><p>
			<?php
			printf(
				/* translators: %d: number of exams written */
				esc_html__( 'Practice Exams published from approved banks. Assessments written/updated: %d. Open the learner Practice Exams page to verify Practice A, Practice B, and the Comprehensive Final.', 'cta-lms' ),
				absint( wp_unslash( $_GET['written'] ?? 0 ) )
			);
			?>
		</p></div>
	<?php elseif ( 'lmft_le_practice_exams_failed' === $notice ) : ?>
		<div class="notice notice-error is-dismissible"><p><?php esc_html_e( 'Could not publish Practice Exams. Confirm quiz seed files are on the server (includes/quiz-seeds/lmft-law-ethics-practice-*.php), then try again.', 'cta-lms' ); ?></p></div>
	<?php elseif ( 'quiz_status_updated' === $notice ) : ?>
...
<<<<<<< HEAD
			<?php if ( $is_exam_prep && $course_id ) : ?>
				...
				$is_lmft_le = $course && ! empty( $course->slug )
					&& 'california-law-ethics-exam-preparation' === sanitize_title( (string) $course->slug );
				$exam_center_quizzes = ! empty( $quizzes ) ? $quizzes : array();
				?>
				<div class="cta-admin-panel" style="margin:0 0 16px;padding:12px 14px;background:#f6f7f7;border:1px solid #dcdcde;">
					<h3 style="margin-top:0;"><?php esc_html_e( 'Practice Exams (Exam Center) — publish / draft', 'cta-lms' ); ?></h3>
					<p class="description" style="margin-top:0;">
						<?php esc_html_e( 'Active = visible to learners on Practice Exams. Draft = hidden. License / workbook banks stay on workbook pages — not here.', 'cta-lms' ); ?>
					</p>
					<?php if ( $is_lmft_le ) : ?>
						<p>
							<a class="button button-primary"
								href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=cta_lmft_le_publish_practice_exams&course_id=' . (int) $course_id ), 'cta_lmft_le_publish_practice_exams' ) ); ?>"
							><?php esc_html_e( 'Publish Practice A + Practice B + Comprehensive Final now', 'cta-lms' ); ?></a>
						</p>
						<p class="description"><?php esc_html_e( 'Loads the approved 50/50/100 question banks into Active Exam Center assessments. Safe to click again (idempotent).', 'cta-lms' ); ?></p>
					<?php endif; ?>
					<?php if ( ! empty( $exam_center_quizzes ) ) : ?>
						<table class="widefat striped" style="margin-top:8px;">
							<thead>
								<tr>
									<th><?php esc_html_e( 'Assessment', 'cta-lms' ); ?></th>
									<th><?php esc_html_e( 'Type', 'cta-lms' ); ?></th>
									<th><?php esc_html_e( 'Status', 'cta-lms' ); ?></th>
									<th><?php esc_html_e( 'Actions', 'cta-lms' ); ?></th>
								</tr>
							...
							<tbody>
								<?php foreach ( $exam_center_quizzes as $qrow ) : ?>
									<?php
									$q_status = (string) ( $qrow->status ?? 'active' );
									$q_type   = (string) ( $qrow->quiz_type ?? '' );
									?>
									<tr>
										<td><strong><?php echo esc_html( (string) $qrow->title ); ?></strong></td>
										<td><code><?php echo esc_html( $q_type ); ?></code></td>
										<td><?php echo 'active' === $q_status ? esc_html__( 'Active (live)', 'cta-lms' ) : esc_html__( 'Draft (hidden)', 'cta-lms' ); ?></td>
										<td>
											<?php if ( 'active' === $q_status ) : ?>
												<a class="button button-small" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=cta_toggle_quiz_status&course_id=' . (int) $course_id . '&quiz_id=' . (int) $qrow->id . '&status=draft' ), 'cta_toggle_quiz_status' ) ); ?>"><?php esc_html_e( 'Set Draft', 'cta-lms' ); ?></a>
											<?php else : ?>
												<a class="button button-small button-primary" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=cta_toggle_quiz_status&course_id=' . (int) $course_id . '&quiz_id=' . (int) $qrow->id . '&status=active' ), 'cta_toggle_quiz_status' ) ); ?>"><?php esc_html_e( 'Publish (Active)', 'cta-lms' ); ?></a>
											<?php endif; ?>
										</td>
									</tr>
								<?php endforeach; ?>
					...
					<?php else : ?>
						<p class="description"><?php esc_html_e( 'No assessments in the database yet for this course.', 'cta-lms' ); ?></p>
					<?php endif; ?>
				</div>
			<?php endif; ?>
