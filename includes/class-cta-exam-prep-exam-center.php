...
<<<<<<< HEAD
			...
			// License / orientation modules are workbook-path content — never Exam Center sims.
			if ( in_array( $type, array( 'license_25', 'start_here', 'orientation' ), true )
				|| false !== stripos( $title, 'Practice Act Module' )
				|| false !== stripos( $title, 'License-Specific Module' )
				|| false !== stripos( $title, 'Start Here' ) ) {
				continue;
			}
