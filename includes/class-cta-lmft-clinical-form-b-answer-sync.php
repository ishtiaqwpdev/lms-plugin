<?php
/**
 * Comprehensive Simulation Form B secured answer-key sync (LMFT California Clinical).
 *
 * PROMPT 19+: merges admin-only correct answers into the active form_b quiz for
...
/**
 * Class CTA_Lmft_Clinical_Form_B_Answer_Sync
 */
if ( ! class_exists( 'CTA_Lmft_Clinical_Form_B_Answer_Sync' ) ) {

class CTA_Lmft_Clinical_Form_B_Answer_Sync {

	const ANSWER_SEED_OPTION    = 'cta_lmft_clinical_form_b_answers_final_aug14_1_0_256';
	const ADMIN_KEY_FILE        = 'includes/quiz-seeds/admin-only/lmft-clinical-form-b-answer-key.php';
	...
	/**
	 * Map order_index => question_code from the learner Form B seed.
	 *
	...
	public static function get_question_code_order_map() {
		...
		$map = array();
		foreach ( CTA_Lmft_Clinical_Form_B_Sync::get_questions() as $index => $row ) {
	...
	public static function should_suppress_learner_answer_reveal( $quiz, $course ) {
		...
		if ( 'form_b' !== sanitize_key( (string) ( $quiz->quiz_type ?? '' ) ) ) {
			return false;
		}
	...
	/**
	 * Find the active rebuilt Form B quiz ID for the LMFT Clinical course.
	 *
	...
	public static function validate_answer_key( array $records ) {
		if ( self::IMPORTED_THROUGH !== count( $records ) ) {
			return new WP_Error(
				...
				sprintf( 'Expected %d secured Form B answer rows.', self::IMPORTED_THROUGH )
			);
		}

		for ( $num = 1; $num <= self::IMPORTED_THROUGH; $num++ ) {
			$code = sprintf( 'CTA-LMFT-CA-FB-%03d', $num );
	...
	/**
	 * Apply secured correct answers to existing learner Form B rows.
	 *
	...
	public static function sync_answer_keys( $force = false ) {
		...
		if ( ! class_exists( 'CTA_Lmft_Clinical_Form_B_Sync' ) ) {
			return array(
				...
				'quiz_id'   => 0,
				'updated'   => 0,
				'message'   => 'form_b_sync_missing',
			);
		}

		$course = CTA_Lmft_Clinical_Form_B_Sync::find_course();
		if ( ! $course ) {
			return array(
				'ok'        => false,
				'course_id' => 0,
		...
		if ( self::TARGET_QUESTION_COUNT !== count( $db_questions ) ) {
			return array(
				...
				'quiz_id'   => $quiz_id,
				'updated'   => 0,
				'message'   => 'learner_form_b_not_loaded',
			);
		}
		...
		$seed = get_option( CTA_Lmft_Clinical_Form_B_Sync::SEED_OPTION );
		if ( is_array( $seed ) ) {
			$seed['answers_through'] = self::IMPORTED_THROUGH;
			update_option( CTA_Lmft_Clinical_Form_B_Sync::SEED_OPTION, $seed, false );
		}
