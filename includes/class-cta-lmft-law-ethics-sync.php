...
<<<<<<< HEAD
		...
		for ( $wb = 1; $wb <= 9; ++$wb ) {
			...
			$seed     = sprintf( 'lmft-law-ethics-wb%d.php', $wb );
			$questions = self::load_seed_questions( $seed );
			$got       = count( $questions );
			if ( $got !== $count ) {
				$result['message'] = sprintf(
					'invalid_question_bank_count:wb%d_bank expected %d got %d',
					$wb,
					$count,
					$got
				);
...
<<<<<<< HEAD
		$practice_a_q = self::load_seed_questions( 'lmft-law-ethics-practice-a.php' );
		$practice_b_q = self::load_seed_questions( 'lmft-law-ethics-practice-b.php' );
		$final_q      = self::load_seed_questions( 'lmft-law-ethics-comprehensive-final.php' );

		if ( 50 !== count( $practice_a_q ) ) {
			$result['message'] = sprintf( 'invalid_question_bank_count:practice_a expected 50 got %d', count( $practice_a_q ) );
			return $result;
		}
		if ( 50 !== count( $practice_b_q ) ) {
			$result['message'] = sprintf( 'invalid_question_bank_count:practice_b expected 50 got %d', count( $practice_b_q ) );
			return $result;
		}
		if ( 100 !== count( $final_q ) ) {
			$result['message'] = sprintf( 'invalid_question_bank_count:comprehensive_final expected 100 got %d', count( $final_q ) );
			return $result;
		}
...
>>>>>>> 1dcdd55b430ec7b912f0b502b3878173ec976d47
	...
	/**
<<<<<<< HEAD
	 * Workbook online Practice Bank definitions (wb1_bank … wb9_bank).
	 *
	 * @return array<int,array<string,mixed>>
	 */
	private static function get_workbook_bank_defs() {
		...
		foreach ( $banks as $wb => $meta ) {
			...
			$defs[] = array(
				...
				'sort'      => $sort,
				'time'      => (int) $meta['time'],
				'file'      => sprintf( 'lmft-law-ethics-wb%d.php', $wb ),
				'expect'    => $expect,
				'key'       => 'wb' . $wb . '_bank',
				'wb_num'    => (int) $wb,
			);
	...
	/**
	 * Live health for one workbook Practice Bank.
	 *
	 * @param int $workbook_num Workbook number 1–9.
	 * @param int $course_id    Optional course ID.
	 * @return array{ok:bool,quiz_id:int,questions:int,expect:int,status:string}
	 */
	public static function get_live_workbook_bank_health( $workbook_num, $course_id = 0 ) {
		$workbook_num = absint( $workbook_num );
		$course_id    = absint( $course_id );
		...
		if ( $workbook_num < 1 || $workbook_num > 9 ) {
			return $empty;
		}
		...
		$expect = 0;
		foreach ( self::get_workbook_bank_defs() as $def ) {
			if ( (int) $def['wb_num'] === $workbook_num ) {
				$expect = (int) $def['expect'];
				break;
			}
		}
		...
		$ok     = ( 'active' === $status && $q_count === $expect && $expect > 0 );
	...
	/**
	 * Whether all 9 workbook Practice Banks are active with expected question counts.
	 *
	 * @param int $course_id Optional course ID.
	 * @return bool
	 */
	...
	/**
	 * Whether every workbook has a printable Candidate Form practice bank attached.
	 *
	 * @param int $course_id Optional course ID.
	 * @return bool
	 */
	public static function workbook_practice_bank_materials_are_live( $course_id = 0 ) {
		if ( ! $course_id ) {
			$course    = self::find_course();
			$course_id = $course ? (int) $course->id : 0;
		}
		...
		$modules   = (array) CTA_Database::get_course_modules( $course_id );

		for ( $n = 1; $n <= 9; ++$n ) {
			$module = null;
			foreach ( $modules as $mod ) {
				if ( preg_match( '/^Workbook\s+' . $n . '\s*:/i', (string) ( $mod->title ?? '' ) ) ) {
					$module = $mod;
					break;
				}
			}
			...
			if ( ! CTA_Exam_Prep_Workbooks::find_practice_bank_resource( $resources, $module ) ) {
				return false;
			}
		}
	...
	/**
	 * Workbook Practice Bank publish status for admin / heal reporting.
	 *
	 * @param int $course_id Optional course ID.
	 * @return array<int,array{title:string,quiz_type:string,online_status:string,online_questions:int,online_expect:int,docx_attached:bool}>
	 */
	public static function get_workbook_bank_status_report( $course_id = 0 ) {
		if ( ! $course_id ) {
			$course    = self::find_course();
			$course_id = $course ? (int) $course->id : 0;
		}
		...
		$titles    = class_exists( 'CTA_Lmft_Law_Ethics_Copy' )
			? CTA_Lmft_Law_Ethics_Copy::workbook_titles()
			: array();
		$resources = $course_id && class_exists( 'CTA_Database' )
			? (array) CTA_Database::get_downloadable_resources( $course_id )
			: array();
		$modules   = $course_id && class_exists( 'CTA_Database' )
			? (array) CTA_Database::get_course_modules( $course_id )
			: array();
		...
		for ( $n = 1; $n <= 9; ++$n ) {
			...
			$docx = $module && class_exists( 'CTA_Exam_Prep_Workbooks' )
				? CTA_Exam_Prep_Workbooks::find_practice_bank_resource( $resources, $module )
				: null;

			$online_status = (string) ( $health['status'] ?? '' );
			if ( '' === $online_status ) {
				$online_status = 'missing';
			} elseif ( ! empty( $health['ok'] ) ) {
				$online_status = 'active';
			} elseif ( 'active' !== $online_status ) {
				$online_status = 'draft';
			}

			$report[ $n ] = array(
				'title'            => (string) ( $titles[ $n ] ?? ( 'Workbook ' . $n ) ),
				'quiz_type'        => 'wb' . $n . '_bank',
				'online_status'    => $online_status,
				'online_questions' => (int) ( $health['questions'] ?? 0 ),
				'online_expect'    => (int) ( $health['expect'] ?? 0 ),
				'docx_attached'    => (bool) $docx,
			);
		}
	...
	/**
	 * Count missing / under-seeded workbook Practice Banks.
	 *
	 * @param int $course_id Course ID.
	 * @return int
	 */
	...
	/**
	 * Sync only workbook online Practice Banks (does not touch Practice Exams / lesson HTML / flashcards).
	 *
	 * @param int $course_id Course ID.
	 * @return array<string,mixed>
	 */
	public static function sync_workbook_banks( $course_id ) {
		...
		$written = 0;
		foreach ( self::get_workbook_bank_defs() as $def ) {
			$questions = self::load_seed_questions( (string) $def['file'] );
			if ( count( $questions ) !== (int) $def['expect'] ) {
				$result['message'] = sprintf(
					'invalid_question_bank_count:%s expected %d got %d',
					$def['quiz_type'],
					$def['expect'],
		...
		$result['ok']      = ( 9 === $written );
		$result['message'] = $result['ok'] ? 'workbook_banks_synced' : 'workbook_bank_sync_failed';
		return $result;
	}

	/**
	 * Sync up to N missing workbook Practice Banks (chunked for deferred upgrades).
	 *
	 * @param int $course_id Optional course ID.
	 * @param int $max_banks Max banks to write this pass.
	 * @return array<string,mixed>
	 */
	public static function sync_workbook_banks_missing( $course_id = 0, $max_banks = 2 ) {
		$course_id = absint( $course_id );
		$max_banks = max( 1, absint( $max_banks ) );
		...
		$written = 0;
		foreach ( self::get_workbook_bank_defs() as $def ) {
			if ( $written >= $max_banks ) {
				break;
			}
			$wb_num = (int) $def['wb_num'];
			$health = self::get_live_workbook_bank_health( $wb_num, $course_id );
			if ( ! empty( $health['ok'] ) ) {
				continue;
			}
			...
			if ( count( $questions ) !== (int) $def['expect'] ) {
				...
				$result['remaining'] = self::count_missing_workbook_banks( $course_id );
				return $result;
			}
			...
			if ( ! $quiz_id ) {
				$result['message']   = 'quiz_write_failed';
				$result['remaining'] = self::count_missing_workbook_banks( $course_id );
				return $result;
			}
			++$written;
	...
	/**
	 * Queue / sync missing workbook Practice Banks (scoped — no Practice Exam writes).
	 *
	 * @param int  $course_id Optional course ID.
	 * @param bool $force     Force queue even if healthy.
	 * @return array<string,mixed>
	 */
	public static function ensure_workbook_banks( $course_id = 0, $force = false ) {
		$course_id = absint( $course_id );
		...
		$sync = self::sync_workbook_banks_missing( $course_id, 9 );
		return array(
			'ok'      => ! empty( $sync['ok'] ) && empty( $sync['remaining'] ),
			'message' => (string) ( $sync['message'] ?? 'workbook_bank_sync_failed' ),
			'queued'  => false,
		);
	}
	...
	/**
	 * Practice Exam definitions (Practice A/B + Comprehensive Final only).
	 *
	 * @return array<int,array<string,mixed>>
	 */
	private static function get_practice_exam_defs() {
	...
	/**
	 * Live health for one Practice Exam quiz.
	 *
	 * @param string $quiz_type Quiz type key.
	 * @param int    $expect    Expected question count.
	 * @param int    $course_id Optional course ID.
	 * @return array{ok:bool,quiz_id:int,questions:int,expect:int,status:string}
	 */
	public static function get_live_practice_exam_health( $quiz_type, $expect, $course_id = 0 ) {
		$quiz_type = sanitize_key( (string) $quiz_type );
		$expect    = absint( $expect );
		$course_id = absint( $course_id );
		$empty     = array(
			'ok'        => false,
			'quiz_id'   => 0,
	...
	/**
	 * Whether Practice A/B + Comprehensive Final are live with expected counts.
	 *
	 * @param int $course_id Optional course ID.
	 * @return bool
	 */
	public static function practice_exams_are_live( $course_id = 0 ) {
		foreach ( self::get_practice_exam_defs() as $def ) {
			$health = self::get_live_practice_exam_health( (string) $def['quiz_type'], (int) $def['expect'], $course_id );
			if ( empty( $health['ok'] ) ) {
				return false;
			}
		}
	...
	/**
	 * Sync only Practice Exams (scoped — no workbook banks / lessons / flashcards).
	 *
	 * @param int $course_id Course ID.
	 * @return array<string,mixed>
	 */
	public static function sync_practice_exams( $course_id ) {
		...
		$result['written'] = $written;
		$result['ok']      = ( 3 === $written );
		$result['message'] = $result['ok'] ? 'practice_exams_synced' : 'practice_exam_sync_failed';
		return $result;
	}

	/**
	 * Sync missing Practice Exams (chunked deferred upgrade helper).
	 *
	 * @param int $course_id Optional course ID.
	 * @param int $max_exams Max exams to write this pass.
	 * @return array<string,mixed>
	 */
	public static function sync_practice_exams_missing( $course_id = 0, $max_exams = 1 ) {
		$course_id = absint( $course_id );
		$max_exams = max( 1, absint( $max_exams ) );
		...
		foreach ( self::get_practice_exam_defs() as $def ) {
			...
			if ( count( $questions ) !== (int) $def['expect'] ) {
				...
				$result['remaining'] = self::count_missing_practice_exams( $course_id );
				return $result;
			}
	...
	/**
	 * Self-heal missing Practice Exams (transient-guarded).
	 *
	 * Never does a heavy DB write on plugins_loaded / plugin upload — that caused
	 * Hostinger white-screens on update.php?action=upload-plugin. Queue deferred
	 * work instead; optional $allow_immediate publishes only from safe contexts
	 * (e.g. Practice Exams page after the plugin is already live).
	 *
	 * @param bool $allow_immediate When true, sync missing exams in this request.
	 * @return void
	 */
	public static function maybe_heal_practice_exams( $allow_immediate = false ) {
		if ( function_exists( 'cta_lms_is_plugin_lifecycle_request' ) && cta_lms_is_plugin_lifecycle_request() ) {
			return;
		}
		if ( get_transient( 'cta_lmft_law_ethics_practice_exam_heal_lock' ) ) {
			return;
		}
		$course = self::find_course();
		if ( ! $course ) {
		...
		$course_id = (int) $course->id;
		if ( self::practice_exams_are_live( $course_id ) ) {
			return;
		}

		set_transient( 'cta_lmft_law_ethics_practice_exam_heal_lock', 1, 5 * MINUTE_IN_SECONDS );
		...
		// Write missing Practice A / B / Final from approved seeds (up to 3 this request on Exam Center).
		self::sync_practice_exams_missing( $course_id, $allow_immediate ? 3 : 1 );

		if ( ! self::practice_exams_are_live( $course_id ) && class_exists( 'CTA_Lms_Deferred_Upgrades' ) ) {
			CTA_Lms_Deferred_Upgrades::queue( 'lmft_law_ethics_practice_exams' );
		}
	}
	 ...
	 *
	 * @param bool $allow_immediate When true, publish banks from a safe learner/admin page load.
	 * @return void
	 */
	public static function maybe_heal_workbook_banks( $allow_immediate = false ) {
		...
		if ( get_transient( 'cta_lmft_law_ethics_wb_bank_heal_lock' ) ) {
			return;
		}
		$course = self::find_course();
		if ( ! $course ) {
		...
		$course_id = (int) $course->id;
		if ( self::workbook_banks_are_live( $course_id ) && self::workbook_practice_bank_materials_are_live( $course_id ) ) {
			return;
		}
		set_transient( 'cta_lmft_law_ethics_wb_bank_heal_lock', 1, 5 * MINUTE_IN_SECONDS );
		if ( class_exists( 'CTA_Lms_Deferred_Upgrades' ) ) {
			CTA_Lms_Deferred_Upgrades::queue( 'lmft_law_ethics_workbook_banks' );
		}
		...
		if ( ! self::workbook_practice_bank_materials_are_live( $course_id ) ) {
			self::sync_materials( true );
		}

		if (
			( ! self::workbook_banks_are_live( $course_id ) || ! self::workbook_practice_bank_materials_are_live( $course_id ) )
			&& class_exists( 'CTA_Lms_Deferred_Upgrades' )
		) {
			CTA_Lms_Deferred_Upgrades::queue( 'lmft_law_ethics_workbook_banks' );
=======
