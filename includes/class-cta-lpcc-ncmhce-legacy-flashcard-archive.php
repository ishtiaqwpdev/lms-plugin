<?php
/**
 * Archive legacy LPCC NCMHCE interactive flashcard deck (132 cards).
 *
 * Replaced by the 180-card Current-Exam Flashcard Study Center deck.
...
class CTA_Lpcc_Ncmhce_Legacy_Flashcard_Archive {

	const ARCHIVE_OPTION         = 'cta_lpcc_ncmhce_legacy_flashcards_archived_1_0_266';
	const ARCHIVED_TITLE_PREFIX  = '[Archived] ';
	const ARCHIVED_RESOURCE_SORT = 910;

	const ACTIVE_LEGACY_JSON_REL   = 'assets/course-materials/lpcc-ncmhce/study-tools/flashcards.json';
	const ARCHIVED_LEGACY_JSON_REL = 'assets/course-materials/lpcc-ncmhce/study-tools/_archived/lpcc-ncmhce-legacy-flashcards-v1.0-132.json';
	...
	public static function legacy_resource_path_markers() {
		return array(
			'study-tools/cta_lpcc_clinical_exam_preparation_flashcard_collection_v1.0.docx',
	...
	public static function is_ncmhce_course( $course ) {
		...
		if ( class_exists( 'CTA_Lpcc_Ncmhce_Legacy_Forms_Archive' ) ) {
			return CTA_Lpcc_Ncmhce_Legacy_Forms_Archive::is_ncmhce_course( $course );
		}

		$slug = sanitize_title( (string) ( $course->slug ?? '' ) );
		return in_array( $slug, array( 'lpcc-ncmhce-exam-preparation', 'lpcc-california-clinical-exam-preparation' ), true );
	}
	...
	public static function blocks_learner_legacy_deck( $course ) {
		if ( ! self::is_ncmhce_course( $course ) ) {
			return false;
		}
	...
	public static function is_archived_legacy_json_path( $path ) {
		...
		return false !== strpos( $norm, 'lpcc-ncmhce-legacy-flashcards-v1.0-132.json' )
			|| ( false !== strpos( $norm, 'lpcc-ncmhce/study-tools/_archived/' ) && false !== strpos( $norm, 'flashcards' ) );
	}
	...
	public static function is_blocked_active_legacy_json_path( $path ) {
		...
		return false !== strpos( $norm, 'lpcc-ncmhce/study-tools/flashcards.json' );
	}
