...
<<<<<<< HEAD
	...
	/**
	 * Whether a quiz is a Law & Ethics chapter-level test (quiz_type wb{N}_c{M}).
	 *
	 * Distinct from single workbook banks (wb{N}_bank) used by clinical programs.
	 *
	 * @param object|null $quiz Quiz row.
	 * @return bool
	 */
	public static function is_chapter_test_quiz( $quiz ) {
		if ( ! $quiz ) {
			return false;
		}

		$type = sanitize_key( (string) ( $quiz->quiz_type ?? '' ) );
		return (bool) preg_match( '/^wb\d+_c\d+$/', $type );
	}

	/**
	 * Chapter number from quiz_type wb{N}_c{M}.
	 *
	 * @param object|null $quiz Quiz row.
	 * @return int
	 */
	public static function chapter_number_from_quiz( $quiz ) {
		...
		if ( preg_match( '/\bC(?:hapter)?\s*(\d+)\b/i', $title, $m ) ) {
			return absint( $m[1] );
		}
	...
	/**
	 * Unique learner-facing label for a chapter test (never a shared workbook bank name).
	 *
	 * Uses approved chapter titles from sync (e.g. WB1-C1 — Foundations… (Chapter Test)).
	 *
	 * @param object|null $quiz Quiz row.
	 * @return string
	 */
	public static function get_chapter_test_display_label( $quiz ) {
		if ( ! $quiz ) {
		...
		$raw = trim( (string) ( $quiz->title ?? '' ) );
		$topic = '';

		if ( preg_match( '/^WB\d+-C\d+\s*[—\-–:]\s*(.+?)(?:\s*\(Chapter Test\))?\s*$/iu', $raw, $m ) ) {
			$topic = trim( (string) $m[1] );
		} elseif ( preg_match( '/Chapter\s+\d+\s*[—\-–:]\s*(.+)$/iu', $raw, $m ) ) {
			$topic = trim( (string) $m[1] );
		}
